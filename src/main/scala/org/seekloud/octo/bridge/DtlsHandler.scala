...
/**
  ...
  * Date on 2019/8/16
  * Time at 13:25
  * implement dtls protocol
  * exchange srtp password
  */
object DtlsHandler {
  ...
  val handlers: mutable.HashMap[String, DtlsHandler] = mutable.HashMap.empty

  def closeHandlers(id: String) = handlers.remove(id).foreach(_.close())
}

class DtlsHandler(session: WebSocketSession) extends IceHandler(session) {
  ...
  override def prepareAnswer(offerSdp: SessionDescription, answerSdp: SessionDescription): SessionDescription = {
    ...
    try {
      ...
      if (globalFingerPrint != null) notifyRemoteFingerprint(null, globalFingerPrint)
        ...
        offerSdp.getMediaDescriptions(false).asInstanceOf[Vector[MediaDescription]].foreach(md => {
          try {
        ...
        answerSdp.getMediaDescriptions(false).asInstanceOf[Vector[MediaDescription]].foreach(md => {
          try {
